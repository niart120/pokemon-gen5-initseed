...
}`;const Ot=/WORKGROUP_SIZE_PLACEHOLDER/g,Bt="seed-search-kernel-module",At="seed-search-kernel",kt="seed-search-kernel-layout",Lt="seed-search-kernel-bind-layout";function Ut(e){return vt.replace(Ot,String(e))}function Ft(e,t){const r=e.createShaderModule({label:Bt,code:Ut(t)});r.getCompilationInfo?.().then(d=>{d.messages.length>0&&console.warn("[seed-search-kernel] compilation diagnostics",d.messages.map(i=>({message:i.message,line:i.lineNum,column:i.linePos,type:i.type})))}).catch(d=>{console.warn("[seed-search-kernel] compilation info failed",d)});const n=e.createBindGroupLayout({label:Lt,entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"read-only-storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),a=e.createPipelineLayout({label:kt,bindGroupLayouts:[n]});return{pipeline:e.createComputePipeline({label:At,layout:a,compute:{module:r,entryPoint:"sha1_generate"}}),layout:n}}const Wt=4,pe=256,se=new Uint32Array([0]);function Me(e,t){const r={context:t??null,pipeline:null,bindGroupLayout:null,targetBuffer:null,targetCapacity:0,workgroupSize:0,candidateCapacity:0,currentLimits:null,dispatchSlots:[],availableSlots:[],slotWaiters:[],desiredDispatchSlots:1},n=async(s,g)=>{r.context||(r.context=await De());const u=r.context.getDevice(),f=r.context.getSupportedWorkgroupSize(s.workgroupSize),h=Math.max(1,g?.dispatchSlots??r.desiredDispatchSlots??1),m=!r.currentLimits||r.workgroupSize!==f||r.candidateCapacity!==s.candidateCapacityPerDispatch;if(!r.pipeline||m){const{pipeline:y,layout:x}=Ft(u,f);r.pipeline=y,r.bindGroupLayout=x}r.workgroupSize=f,r.candidateCapacity=s.candidateCapacityPerDispatch,r.currentLimits=s,r.desiredDispatchSlots=h,a(u,h,s.candidateCapacityPerDispatch),r.currentLimits=s},a=(s,g,u)=>{for(const f of r.dispatchSlots)c(s,f,u);for(;r.dispatchSlots.length<g;){const f=r.dispatchSlots.length,h=o(s,f,u);r.dispatchSlots.push(h)}for(;r.dispatchSlots.length>g;){const f=r.dispatchSlots.pop();f&&l(f)}r.availableSlots=[...r.dispatchSlots],r.slotWaiters.length=0},o=(s,g,u)=>{const f=new Uint32Array(Wt),h=Y(f.byteLength),m=s.createBuffer({label:`seed-search-dispatch-state-${g}`,size:h,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),{matchOutputBuffer:S,readbackBuffer:y,matchBufferSize:x}=i(s,u,g);return{id:g,dispatchStateBuffer:m,dispatchStateData:f,uniformBuffer:null,uniformCapacityWords:0,matchOutputBuffer:S,readbackBuffer:y,matchBufferSize:x}},d=(s,g,u)=>{const f=Y(u*Uint32Array.BYTES_PER_ELEMENT);(!g.uniformBuffer||g.uniformCapacityWords<u)&&(g.uniformBuffer?.destroy(),g.uniformBuffer=s.createBuffer({label:`seed-search-uniform-${g.id}`,size:f,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),g.uniformCapacityWords=u)},i=(s,g,u)=>{const f=j+g*K,h=Y(f*Uint32Array.BYTES_PER_ELEMENT),m=s.createBuffer({label:`seed-search-output-${u}`,size:h,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),S=s.createBuffer({label:`seed-search-readback-${u}`,size:h,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST});return{matchOutputBuffer:m,readbackBuffer:S,matchBufferSize:h}},c=(s,g,u)=>{const f=j+u*K,h=Y(f*Uint32Array.BYTES_PER_ELEMENT);if(g.matchBufferSize===h)return;g.matchOutputBuffer.destroy(),g.readbackBuffer.destroy();const m=i(s,u,g.id);g.matchOutputBuffer=m.matchOutputBuffer,g.readbackBuffer=m.readbackBuffer,g.matchBufferSize=m.matchBufferSize},l=s=>{s.dispatchStateBuffer.destroy(),s.uniformBuffer?.destroy(),s.matchOutputBuffer.destroy(),s.readbackBuffer.destroy()},p=()=>r.availableSlots.length>0?Promise.resolve(r.availableSlots.pop()):new Promise(s=>{r.slotWaiters.push(s)}),_=s=>{const g=r.slotWaiters.shift();if(g){g(s);return}r.availableSlots.push(s)};return{ensureConfigured:n,setTargetSeeds:s=>{if(!r.context)throw new Error("SeedSearchEngine is not configured yet");const g=r.context.getDevice(),u=s.length,f=1+u,h=Y(f*Uint32Array.BYTES_PER_ELEMENT);(!r.targetBuffer||r.targetCapacity<u)&&(r.targetBuffer?.destroy(),r.targetBuffer=g.createBuffer({label:"seed-search-target-seeds",size:h,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),r.targetCapacity=u);const m=new Uint32Array(f);m[0]=u>>>0;for(let S=0;S<u;S+=1)m[1+S]=s[S]>>>0;g.queue.writeBuffer(r.targetBuffer,0,m.buffer,m.byteOffset,m.byteLength)},executeSegment:async s=>{if(!r.context||!r.pipeline||!r.bindGroupLayout)throw new Error("SeedSearchEngine is not ready");if(!r.targetBuffer)throw new Error("Target seed buffer is not prepared");if(r.dispatchSlots.length===0)throw new Error("Dispatch slots are not configured");const g=r.context.getDevice(),u=g.queue,f=Math.max(1,s.workgroupCount),h=f,m=await p();try{const S=J();u.writeBuffer(m.matchOutputBuffer,0,se.buffer,se.byteOffset,se.byteLength);const y=m.dispatchStateData;y[0]=s.messageCount>>>0,y[1]=s.baseSecondOffset>>>0,y[2]=r.candidateCapacity>>>0,y[3]=0,u.writeBuffer(m.dispatchStateBuffer,0,y.buffer,y.byteOffset,y.byteLength);const x=s.getUniformWords();d(g,m,x.length),u.writeBuffer(m.uniformBuffer,0,x.buffer,x.byteOffset,x.byteLength);const O=J(),D=g.createBindGroup({label:`seed-search-bind-group-${s.id}-slot-${m.id}`,layout:r.bindGroupLayout,entries:[{binding:0,resource:{buffer:m.dispatchStateBuffer}},{binding:1,resource:{buffer:m.uniformBuffer}},{binding:2,resource:{buffer:r.targetBuffer}},{binding:3,resource:{buffer:m.matchOutputBuffer}}]}),R=g.createCommandEncoder({label:`seed-search-encoder-${s.id}`}),B=R.beginComputePass({label:`seed-search-pass-${s.id}`});B.setPipeline(r.pipeline),B.setBindGroup(0,D),B.dispatchWorkgroups(f),B.end(),R.copyBufferToBuffer(m.matchOutputBuffer,0,m.readbackBuffer,0,m.matchBufferSize);const U=R.finish();u.submit([U]),await m.readbackBuffer.mapAsync(GPUMapMode.READ,0,m.matchBufferSize);const A=J(),re=m.readbackBuffer.getMappedRange(0,m.matchBufferSize),W=new Uint32Array(re.slice(0));m.readbackBuffer.unmap();const I=J(),V=W[0]??0,z=Math.min(V,r.candidateCapacity),ne=Math.min(W.length,j+z*K),q={words:W.slice(0,ne),matchCount:z};return e?.onDispatchComplete?.({segmentId:s.id,messageCount:s.messageCount,workgroupCount:h,matchCount:z,candidateCapacity:r.candidateCapacity,timings:{totalMs:I-S,setupMs:O-S,gpuMs:A-O,readbackMs:I-A},timestampMs:I}),q}finally{_(m)}},dispose:()=>{for(const s of r.dispatchSlots)l(s);r.dispatchSlots=[],r.availableSlots=[],r.slotWaiters.length=0,r.targetBuffer?.destroy(),r.context=null,r.pipeline=null,r.bindGroupLayout=null,r.targetBuffer=null,r.targetCapacity=0,r.currentLimits=null},getWorkgroupSize:()=>r.workgroupSize,getCandidateCapacity:()=>r.candidateCapacity,getSupportedLimits:()=>r.context?.getLimits()??null}}function Y(e){return Math.ceil(e/pe)*pe}function J(){return typeof performance<"u"?performance.now():Date.now()}const It=1024,zt=500;function Gt(e){const t=new lt,r=e??Me(),n={isRunning:!1,isPaused:!1,shouldStop:!1,job:null,progress:null,callbacks:null,timer:{cumulativeRunTime:0,segmentStartTime:0,isPaused:!1},lastProgressUpdate:0},a=async(u,f,h)=>{if(n.isRunning)throw new Error("Seed search is already running");n.isRunning=!0,n.isPaused=!1,n.shouldStop=h?.aborted??!1,n.job=u,n.callbacks=f,n.lastProgressUpdate=0,n.progress={currentStep:0,totalSteps:u.summary.totalMessages,elapsedTime:0,estimatedTimeRemaining:0,matchesFound:0,currentDateTime:u.timePlan?new Date(u.timePlan.startDayTimestampMs).toISOString():void 0},b(),_(!1);let m;if(h){const S=()=>{n.shouldStop=!0};h.addEventListener("abort",S),m=()=>h.removeEventListener("abort",S),n.abortCleanup=m}try{if(u.summary.totalMessages===0){f.onComplete("探索対象の組み合わせが存在しません");return}const S=Math.max(1,Math.min(u.limits.maxDispatchesInFlight??1,u.segments.length||1));await r.ensureConfigured(u.limits,{dispatchSlots:S}),r.setTargetSeeds(u.targetSeeds);const y=new Set,x=new Set,O=D=>{const R=(async()=>{if(n.shouldStop)return;const{words:B,matchCount:U}=await r.executeSegment(D);if(n.shouldStop)return;const A=(async()=>{await d(D,B,U)})();x.add(A),A.finally(()=>x.delete(A))})();y.add(R),R.finally(()=>y.delete(R))};for(const D of u.segments){if(n.shouldStop||(await p(),n.shouldStop))break;O(D),y.size>=S&&await Promise.race(y)}y.size>0&&await Promise.all(y),x.size>0&&await Promise.all(x),o()}catch(S){const y=S instanceof Error?S.message:"WebGPU検索中に不明なエラーが発生しました",x=globalThis.GPUValidationError,O=x&&S instanceof x?"WEBGPU_VALIDATION_ERROR":void 0;throw f.onError(y,O),S}finally{m?.(),n.abortCleanup=void 0,n.isRunning=!1,n.isPaused=!1,E(),n.job=null,n.callbacks=null,n.progress=null,n.shouldStop=!1,n.lastProgressUpdate=0}},o=()=>{const u=n.callbacks,f=n.progress;if(!(!u||!f)){if(w(f),n.shouldStop){u.onStopped("検索を停止しました",f);return}u.onProgress(f),u.onComplete(`検索が完了しました。${f.matchesFound}件ヒットしました。`)}},d=async(u,f,h)=>{const m=n.job,S=n.callbacks,y=n.progress;if(!m||!S||!y)return;const x=j,O=K;for(let D=0;D<h&&!(n.shouldStop||D%It===0&&(await p(),n.shouldStop));D+=1){const R=x+D*O,B=f[R],U=f[R+1]>>>0,A=u.globalMessageOffset+B,re=u.baseSecondOffset+B,W=u.timer0,I=u.vcount,V=ce(m.timePlan,re),z=t.generateMessage(m.conditions,W,I,V,u.keyCode),{hash:ne,seed:q,lcgSeed:Be}=t.calculateSeed(z);q!==U&&console.warn("GPU/CPU seed mismatch detected",{gpuSeed:U,cpuSeed:q,messageIndex:A});const Ae={seed:U,datetime:V,timer0:W,vcount:I,keyCode:u.keyCode,conditions:m.conditions,message:z,sha1Hash:ne,lcgSeed:Be,isMatch:!0};S.onResult(Ae),y.matchesFound+=1}if(u.messageCount>0){const D=u.messageCount-1,R=u.baseSecondOffset+D;y.currentDateTime=ce(m.timePlan,R).toISOString()}y.currentStep+=u.messageCount,_(!0)},i=()=>{!n.isRunning||n.isPaused||(n.isPaused=!0,C(),n.callbacks?.onPaused())},c=()=>{!n.isRunning||!n.isPaused||(n.isPaused=!1,P(),n.callbacks?.onResumed())},l=()=>{n.isRunning&&(n.shouldStop=!0,n.isPaused=!1,P())},p=async()=>{for(;n.isPaused&&!n.shouldStop;)await g(25)},_=u=>{const f=n.callbacks,h=n.progress;if(!f||!h)return;const m=Date.now();u&&h.currentStep<h.totalSteps&&m-n.lastProgressUpdate<zt||(w(h),f.onProgress(h),n.lastProgressUpdate=m)},w=u=>{const f=v();u.elapsedTime=f,u.estimatedTimeRemaining=s(u.currentStep,u.totalSteps,f)},b=()=>{n.timer.cumulativeRunTime=0,n.timer.segmentStartTime=Date.now(),n.timer.isPaused=!1},C=()=>{n.timer.isPaused||(n.timer.cumulativeRunTime+=Date.now()-n.timer.segmentStartTime,n.timer.isPaused=!0)},P=()=>{n.timer.isPaused&&(n.timer.segmentStartTime=Date.now(),n.timer.isPaused=!1)},E=()=>{n.timer.isPaused||(n.timer.cumulativeRunTime+=Date.now()-n.timer.segmentStartTime,n.timer.isPaused=!0)},v=()=>n.timer.isPaused?n.timer.cumulativeRunTime:n.timer.cumulativeRunTime+(Date.now()-n.timer.segmentStartTime),s=(u,f,h)=>{if(u===0||u>=f)return 0;const m=h/u;return Math.round(m*(f-u))},g=u=>new Promise(f=>setTimeout(f,u));return{run:a,pause:i,resume:c,stop:l}}const ve=self,Nt=3n,he=0x100000000n,M={isRunning:!1,isPaused:!1};let F=null,te=null,ue=null,X=null;function Yt(e,t){if(t<=0||e.maxMessagesPerDispatch<=0)return e.candidateCapacityPerDispatch;const r=Nt*BigInt(e.maxMessagesPerDispatch)*BigInt(t),n=Number((r+he-1n)/he);return Math.max(1,n)}async function Oe(){return ue||(ue=De()),ue}async function Ht(){if(F)return F;const e=await Oe(),t=Me(void 0,e);return F=Gt(t),F}async function Kt(){return X||(X=(await Oe()).deriveSearchJobLimits(),X)}function T(e){ve.postMessage(e)}function $t(){T({type:"READY",message:"WebGPU worker initialized"})}function H(){M.isRunning=!1,M.isPaused=!1,te=null}function Vt(){return pt()?!0:(T({type:"ERROR",error:"WebGPU is not supported in this environment",errorCode:"WEBGPU_UNSUPPORTED"}),!1)}async function qt(e){if(M.isRunning){T({type:"ERROR",error:"Search is already running"});return}if(!e.conditions||!e.targetSeeds){T({type:"ERROR",error:"Missing conditions or target seeds"});return}if(!Vt())return;M.isRunning=!0,M.isPaused=!1;let t,r;try{const[a,o]=await Promise.all([Kt(),Ht()]),d=Yt(a,e.targetSeeds.length),i={...a,candidateCapacityPerDispatch:Math.min(a.candidateCapacityPerDispatch,d)};t=He(e.conditions,e.targetSeeds,{limits:i}),r=o}catch(a){H();const o=a instanceof Error?a.message:"検索条件の解析中にエラーが発生しました";T({type:"ERROR",error:o,errorCode:"WEBGPU_CONTEXT_ERROR"});return}te=new AbortController;const n={onProgress:a=>{T({type:"PROGRESS",progress:a})},onResult:a=>{T({type:"RESULT",result:a})},onComplete:a=>{H(),T({type:"COMPLETE",message:a})},onError:(a,o)=>{H(),T({type:"ERROR",error:a,errorCode:o})},onPaused:()=>{M.isPaused=!0,T({type:"PAUSED"})},onResumed:()=>{M.isPaused=!1,T({type:"RESUMED"})},onStopped:(a,o)=>{H(),T({type:"STOPPED",message:a,progress:o})}};try{await r.run(t,n,te.signal)}catch(a){if(!M.isRunning)return;H();const o=a instanceof Error?a.message:"WebGPU search failed with unknown error";T({type:"ERROR",error:o,errorCode:"WEBGPU_RUNTIME_ERROR"})}}function Jt(){!M.isRunning||M.isPaused||F?.pause()}function Xt(){!M.isRunning||!M.isPaused||F?.resume()}function Zt(){M.isRunning&&(F?.stop(),te?.abort())}$t();ve.onmessage=e=>{const t=e.data;switch(t.type){case"START_SEARCH":qt(t);break;case"PAUSE_SEARCH":Jt();break;case"RESUME_SEARCH":Xt();break;case"STOP_SEARCH":Zt();break;default:T({type:"ERROR",error:`Unknown request type: ${t.type}`})}};
