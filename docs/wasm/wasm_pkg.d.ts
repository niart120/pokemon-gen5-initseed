 ...
 */
export function generate_egg_key_codes(key_input_mask: number): Uint32Array;
/**
 * オフセット計算統合API（仕様書準拠）
 */
...
export class EggBootTimingSearchIterator {
   ...
   */
  constructor(mac: Uint8Array, nazo: Uint32Array, hardware: string, timer0: number, vcount: number, key_code: number, hour_start: number, hour_end: number, minute_start: number, minute_end: number, second_start: number, second_end: number, year_start: number, month_start: number, date_start: number, range_seconds: number, conditions: GenerationConditionsJs, parents: ParentsIVsJs, filter_js: IndividualFilterJs | null | undefined, consider_npc_consumption: boolean, game_mode: GameMode, user_offset: bigint, advance_count: number);
  /**
   * 次のバッチを取得
   *
...
export interface InitOutput {
  ...
  readonly generate_egg_key_codes: (a: number, b: number) => void;
  ...
  readonly eggboottimingsearchiterator_new: (a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number, i: number, j: number, k: number, l: number, m: number, n: number, o: number, p: number, q: number, r: number, s: number, t: number, u: number, v: number, w: number, x: number, y: number, z: bigint, a1: number) => void;
