...
- [乱数調整　6Vメタモン　解説（milk4724様）](https://milk4724.hatenablog.com/entry/20231230/00000000)
...
**Auto機能を使用する場合（推奨）**:
- 「Auto」スイッチをONにすると、ROM設定から標準値が自動設定される
- 初めての場合はこの設定で開始し、結果を見て調整する

**手動で指定する場合**:
- 「Auto」スイッチをOFFにして、特定した値を入力
- Timer0範囲（例: `0C67`〜`0C69`）
- VCount範囲（例: `5F`〜`5F`）
...
1. 「Target Seeds」カード内の「**Template**」ボタンをクリック
2. 「Seedテンプレートの選択」ダイアログが表示される
3. 「**BW 固定・野生 6V**」にチェックを入れる
4. 「**選択を適用**」ボタンをクリック

![Target Seeds Loaded](./images/03-target-seeds-loaded.png)

Seed入力欄に5件のSeedが読み込まれます（例: `0x14B11BA6` 等）。
...
- 例: 時間範囲を`11`～`11`に固定する場合、4～5キーを選択するとよいでしょう。
...
画面上部のタブで「**Generation**」をクリックする。

![Generation Panel](./images/04-generation-panel.png)
...
- **Boot-Timing**: 起動日時・キー入力から計算する場合（検索結果を使用する際はこちら）

### 4.3 エンカウント設定

![Generation Encounter Settings](./images/06-generation-encounter-settings.png)
...
1. DS本体の設定画面で日時を変更
2. 検索結果の日時に設定
   - 例: 結果が `2012/03/02 08:31:11` の場合、 日付を`2012/03/02`、時刻を`08:31` に設定

### 5.2 ソフト起動

1. DS本体メニューでBWを選択
2. **目標時刻の1秒前**（例: `08:31:10`）にソフトを選択

### 5.3 白い画面中の操作

ソフト選択後、約5秒間白い画面が表示されます。この間に**指定されたキーを押し続けてください**。

例: `A`, `B`, `→` の場合、3つのボタンを同時に長押しする(Nintendoロゴが消えるまで押し続ける)
