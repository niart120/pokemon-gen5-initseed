    ...
    fn slot_value_to_encounter_slot(encounter_type: EncounterType, slot_value: u32) -> u8 {
        match encounter_type {
            ...
            EncounterType::ShakingGrass => {
                Self::calculate_shaking_grass_encounter_from_slot(slot_value)
            }
            EncounterType::DustCloud => Self::calculate_dust_cloud_encounter_from_slot(slot_value),
            EncounterType::PokemonShadow => {
                Self::calculate_pokemon_shadow_encounter_from_slot(slot_value)
            }
            EncounterType::SurfingBubble => {
                Self::calculate_surfing_bubble_encounter_from_slot(slot_value)
            }
            EncounterType::FishingBubble => {
                Self::calculate_fishing_bubble_encounter_from_slot(slot_value)
            }
...
mod tests {
    ...
    fn test_normal_encounter_distribution() {
        let dist =
            EncounterCalculator::calculate_slot_distribution(EncounterType::Normal, GameVersion::B);
    ...
    fn test_slot_to_table_index() {
        // 通常エンカウント
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::Normal, 5),
            5
        );
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::Normal, 15),
            11
        );

        // なみのり
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::Surfing, 3),
            3
        );
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::Surfing, 10),
            4
        );

        // つりざお
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::Fishing, 2),
            2
        );
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::Fishing, 8),
            4
        );

        // 揺れる草むら
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::ShakingGrass, 1),
            1
        );
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::ShakingGrass, 5),
            4
        );

        // 砂煙
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::DustCloud, 1),
            1
        );
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::DustCloud, 5),
            2
        );

        // ポケモンの影
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::PokemonShadow, 2),
            2
        );
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::PokemonShadow, 5),
            3
        );

        // 水泡なみのり
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::SurfingBubble, 1),
            1
        );
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::SurfingBubble, 6),
            3
        );

        // 水泡釣り
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::FishingBubble, 2),
            2
        );
        assert_eq!(
            EncounterCalculator::slot_to_table_index(EncounterType::FishingBubble, 7),
            3
        );
    }
    ...
    fn test_version_mathematical_differences() {
        ...
        for &rand_val in &test_values {
            let bw_slot =
                EncounterCalculator::calculate_raw_encounter_slot(GameVersion::B, rand_val);
            let bw2_slot =
                EncounterCalculator::calculate_raw_encounter_slot(GameVersion::B2, rand_val);
            ...
            for encounter_type in encounter_types {
                for &rand_val in &test_values {
                    ...
                    let max_slot = match encounter_type {
                        ...
                        EncounterType::Surfing
                        | EncounterType::Fishing
                        | EncounterType::ShakingGrass => 4,
                        EncounterType::DustCloud => 2,
                        EncounterType::PokemonShadow
                        | EncounterType::SurfingBubble
                        | EncounterType::FishingBubble => 3,
                        EncounterType::StaticSymbol
                        | EncounterType::StaticStarter
                        | EncounterType::StaticFossil
                        | EncounterType::StaticEvent
