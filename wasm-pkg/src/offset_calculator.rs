...
impl TidSidResult {
    ...
    #[wasm_bindgen(getter)]
    pub fn get_advances_used(&self) -> u32 {
        self.advances_used
    }
}
...
mod tests {
    ...
    fn test_bw1_new_game_no_save_seed_0x12345678() {
        ...
        assert_eq!(
            offset, 71,
            "BW1 最初から（セーブデータなし）のオフセットが一致しません"
        );
    ...
    fn test_bw1_new_game_with_save_seed_0x12345678() {
        ...
        assert_eq!(
            offset, 59,
            "BW1 最初から（セーブデータあり）のオフセットが一致しません"
        );
    ...
    fn test_bw2_new_game_no_save_seed_0x90abcdef() {
        ...
        assert_eq!(
            offset, 44,
            "BW2 最初から（セーブデータなし）のオフセットが一致しません"
        );

        let tid_sid = calculate_tid_sid_from_seed(seed, GameMode::Bw2NewGameNoSave);
        assert_eq!(
            tid_sid.tid, 910,
            "BW2 最初から（セーブデータなし）のTIDが一致しません"
        );
        assert_eq!(
            tid_sid.sid, 42056,
            "BW2 最初から（セーブデータなし）のSIDが一致しません"
        );
    }
    ...
    fn test_bw2_new_game_no_memory_link_save_seed_0x90abcdef() {
        ...
        assert_eq!(
            offset, 29,
            "BW2 最初から（セーブデータあり、思い出リンクなし）のオフセットが一致しません"
        );

        let tid_sid = calculate_tid_sid_from_seed(seed, GameMode::Bw2NewGameNoMemoryLinkSave);
        assert_eq!(
            tid_sid.tid, 4043,
            "BW2 最初から（セーブデータあり、思い出リンクなし）のTIDが一致しません"
        );
        assert_eq!(
            tid_sid.sid, 13882,
            "BW2 最初から（セーブデータあり、思い出リンクなし）のSIDが一致しません"
        );
    }
    ...
    fn test_bw2_new_game_with_memory_link_save_seed_0x90abcdef() {
        ...
        assert_eq!(
            offset, 29,
            "BW2 最初から（セーブデータあり、思い出リンクあり）のオフセットが一致しません"
        );

        let tid_sid = calculate_tid_sid_from_seed(seed, GameMode::Bw2NewGameWithMemoryLinkSave);
        assert_eq!(
            tid_sid.tid, 4043,
            "BW2 最初から（セーブデータあり、思い出リンクあり）のTIDが一致しません"
        );
        assert_eq!(
            tid_sid.sid, 13882,
            "BW2 最初から（セーブデータあり、思い出リンクあり）のSIDが一致しません"
        );
    }
    ...
    fn test_bw2_continue_no_memory_link_seed_0x90abcdef() {
        ...
        assert_eq!(
            offset, 55,
            "BW2 続きから（思い出リンクなし）のオフセットが一致しません"
        );
    ...
    fn test_bw2_continue_with_memory_link_seed_0x90abcdef() {
        ...
        assert_eq!(
            offset, 55,
            "BW2 続きから（思い出リンクあり）のオフセットが一致しません"
        );
    ...
    fn test_calculate_tid_sid_bw1_no_save_seed_0x12345678() {
        ...
        assert_eq!(
            tid_sid.tid, 5432,
            "BW1 最初から（セーブなし）のTIDが一致しません (seed 0x12345678)"
        );
        assert_eq!(
            tid_sid.sid, 12449,
            "BW1 最初から（セーブなし）のSIDが一致しません (seed 0x12345678)"
        );

        // TID/SID決定完了時点での累積進行数（リファレンス実装のoffset値）
        assert_eq!(
            tid_sid.advances_used, 29,
    ...
    fn test_calculate_tid_sid_bw1_with_save_seed_0x12345678() {
        ...
        assert_eq!(
            tid_sid.tid, 58399,
            "BW1 最初から（セーブあり）のTIDが一致しません (seed 0x12345678)"
        );
        assert_eq!(
            tid_sid.sid, 27333,
            "BW1 最初から（セーブあり）のSIDが一致しません (seed 0x12345678)"
        );
