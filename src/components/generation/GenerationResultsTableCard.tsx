...
export const GenerationResultsTableCard: React.FC = () => {
  ...
  const filtered = filters.shinyOnly ? results.filter(r=> r.shiny_type!==0) : results;
  return (
    <Card className="p-3 flex flex-col gap-2 flex-1 min-h-0">
      <CardHeader className="py-0"><CardTitle className="text-sm">Results ({filtered.length})</CardTitle></CardHeader>
