...
function BigIntInput({ label, value, onChange }: { label: string; value: bigint; onChange: (v:bigint)=>void }) {
  return (
    <label className="flex flex-col gap-1 text-xs font-medium">
      <span>{label}</span>
      <input
        ...
        onChange={e => {
          ...
          try {
            const bi = txt.startsWith('0x') ? BigInt(txt) : BigInt('0x'+txt);
            onChange(bi);
          } catch {/* ignore */}
        }}
      />
    ...
    <div className="flex flex-col gap-3 h-full min-h-0">
      ...
      <div className="border rounded-lg p-3 flex flex-wrap gap-4">
        ...
        <label className="flex flex-col gap-1 text-xs font-medium w-28">
          ...
          <select
            className="border rounded px-2 py-1 text-sm bg-background"
            value={draftParams.version as string}
            disabled={disabledInputs}
            onChange={e=>setDraftParams({ version: e.target.value as any })}
          >
            {['B','W','B2','W2'].map(v=> <option key={v} value={v}>{v}</option>)}
          </select>
        </label>
        <BigIntInput label="Base Seed (hex)" value={draftParams.baseSeed as bigint} onChange={v=>setDraftParams({ baseSeed: v })} />
        <BigIntInput label="Offset" value={draftParams.offset as bigint} onChange={v=>setDraftParams({ offset: v })} />
        <NumberInput label="Max Advances" value={draftParams.maxAdvances as number} onChange={v=>setDraftParams({ maxAdvances: v })} />
        <NumberInput label="Max Results" value={draftParams.maxResults as number} onChange={v=>setDraftParams({ maxResults: v })} />
        <NumberInput label="Batch Size" value={draftParams.batchSize as number} onChange={v=>setDraftParams({ batchSize: v })} />
        <NumberInput label="TID" value={draftParams.tid as number} onChange={v=>setDraftParams({ tid: v })} />
        <NumberInput label="SID" value={draftParams.sid as number} onChange={v=>setDraftParams({ sid: v })} />
        {/* Encounter Type */}
        <label className="flex flex-col gap-1 text-xs font-medium w-40">
          <span>Encounter</span>
          <select
            className="border rounded px-2 py-1 text-sm bg-background"
            value={draftParams.encounterType as number}
            disabled={disabledInputs}
        ...
        <label className="flex items-center gap-2 text-xs font-medium w-full sm:w-auto">
          <input type="checkbox" checked={draftParams.syncEnabled as boolean} disabled={disabledInputs} onChange={e=>setDraftParams({ syncEnabled: e.target.checked })} />
          Sync Enabled
        </label>
        <label className="flex flex-col gap-1 text-xs font-medium w-36">
          ...
          <select
            className="border rounded px-2 py-1 text-sm bg-background"
            value={draftParams.syncNatureId as number}
            disabled={disabledInputs || !(draftParams.syncEnabled as boolean)}
        ...
        <label className="flex items-center gap-2 text-xs font-medium">
          <input type="checkbox" checked={draftParams.stopAtFirstShiny as boolean} onChange={e=>setDraftParams({ stopAtFirstShiny: e.target.checked })} />
          Stop at First Shiny
        </label>
        <label className="flex items-center gap-2 text-xs font-medium">
          <input type="checkbox" checked={draftParams.stopOnCap as boolean} onChange={e=>setDraftParams({ stopOnCap: e.target.checked })} />
          Stop On Cap
        </label>
        <label className="flex items-center gap-2 text-xs font-medium">
