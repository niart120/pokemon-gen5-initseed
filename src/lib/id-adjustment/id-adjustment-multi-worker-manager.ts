/**
 * ID Adjustment Multi Worker Manager - 並列Worker版
 * ID調整検索の並列Worker管理
 */
...
import { isIdAdjustmentWorkerResponse } from '@/types/id-adjustment-search';
import {
 ...
 * 集約された進捗状態
 */
export interface AggregatedIdAdjustmentProgress {
 ...
 * コールバック定義
 */
export interface IdAdjustmentMultiWorkerCallbacks {
  onProgress: (progress: AggregatedIdAdjustmentProgress) => void;
  onResult: (result: IdAdjustmentSearchResult) => void;
 ...
 * 並列 Worker 管理システム
 */
export class IdAdjustmentMultiWorkerManager {
  ...
  private callbacks: IdAdjustmentMultiWorkerCallbacks | null = null;
  ...
  async startParallelSearch(
    ...
    callbacks: IdAdjustmentMultiWorkerCallbacks
  ): Promise<void> {
    if (this.searchRunning) {
    ...
    try {
      // チャンク分割
      const chunks = calculateIdAdjustmentTimeChunks(params, this.maxWorkers);
  ...
  private async initializeWorker(
    ...
    const worker = new Worker(
      new URL('../../workers/id-adjustment-worker.ts', import.meta.url),
      { type: 'module' }
    );

    worker.onmessage = (event: MessageEvent<IdAdjustmentWorkerResponse>) => {
    ...
    const chunkParams: IdAdjustmentSearchParams = {
      ...params,
      dateRange: {
  ...
  private handleWorkerMessage(workerId: number, data: unknown): void {
    ...
    if (!isIdAdjustmentWorkerResponse(data)) return;
  ...
  private updateWorkerProgress(
    workerId: number,
    progressData: IdAdjustmentProgress
  ): void {
  ...
  private aggregateAndReportProgress(): void {
    ...
    const aggregatedProgress: AggregatedIdAdjustmentProgress = {
      totalCurrentStep,
      totalSteps,
      totalElapsedTime,
