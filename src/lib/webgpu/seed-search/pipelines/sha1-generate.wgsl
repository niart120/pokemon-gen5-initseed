...
fn sha1_generate(@builtin(global_invocation_id) global_id : vec3<u32>) {
  ...
  let seed = compute_seed_from_hash(h0, h1);

  let target_count = target_seeds.count;
  var matched = target_count == 0u;
  for (var i = 0u; i < target_count; i = i + 1u) {
    if (target_seeds.values[i] == seed) {
      matched = true;
      break;
    }
  }
  ...
  let capacity = arrayLength(&output_buffer.records);
  if (record_index >= capacity || record_index >= config.message_count) {
    return;
  }
