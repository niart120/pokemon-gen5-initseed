/**
 * ID Adjustment Worker - ID調整検索用Worker
 *
 * 指定したTID/SIDを持つ起動時間条件を検索する。
 * セグメントベースのパターンを採用:
 * - TypeScript側で timer0 × vcount × keyCode のセグメントループを実装
 * - 各セグメントに対して IdAdjustmentSearchIterator を作成
 * - 結果はストリーミングで送信
 */
import type {
  IdAdjustmentWorkerRequest,
  IdAdjustmentWorkerResponse,
  IdAdjustmentSearchParams,
  IdAdjustmentSearchResult,
  IdAdjustmentCompletion,
  IdAdjustmentProgress,
  WasmIdAdjustmentSearchResult,
...
function buildTimeRangeParams(
  wasmAny: WasmAny,
  timeRange: IdAdjustmentSearchParams['timeRange']
): TimeRangeParamsJs {
...
const post = (message: IdAdjustmentWorkerResponse) => ctx.postMessage(message);

post({ type: 'READY', version: '1' });

ctx.onmessage = (ev: MessageEvent<IdAdjustmentWorkerRequest>) => {
  const msg = ev.data;
  (async () => {
    try {
      switch (msg.type) {
 ...
 * 一時停止中の進捗情報を作成
 */
function createPausedProgress(): IdAdjustmentProgress {
  return {
...
async function handleStart(params: IdAdjustmentSearchParams) {
  if (state.running) {
    return;
  }
  ...
  try {
    ...
    // 完了通知
    const completion: IdAdjustmentCompletion = {
...
function resolveNazoValue(
  romVersion: IdAdjustmentSearchParams['romVersion'],
  romRegion: IdAdjustmentSearchParams['romRegion']
): readonly number[] {
...
function convertWasmResult(
  wasmResult: WasmIdAdjustmentSearchResult,
  ...
  return {
    ...
    tid: wasmResult.tid,
    sid: wasmResult.sid,
    shinyType: wasmResult.shinyType as DomainShinyType,
  };
}
 ...
 * 各セグメントに対して IdAdjustmentSearchIterator を作成する。
 */
async function executeSearch(
  ...
  if (!wasmAny.IdAdjustmentSearchIterator) {
    throw new Error('IdAdjustmentSearchIterator not exposed in WASM');
  }
  ...
  for (
    ...
    for (
      ...
      for (const keyCode of keyCodes) {
        ...
        try {
          iterator = new wasmAny.IdAdjustmentSearchIterator(
            dsConfig,
            segmentParams,
            timeRangeParams,
            searchRangeParams,
          ...
          // エラーが発生した場合はスキップ
          console.error('Failed to create IdAdjustmentSearchIterator:', e);
          processedSegments++;
          continue;
        }
        ...
        try {
          ...
          while (!iterator.isFinished && resultsCount < params.maxResults) {
            ...
            if (resultsArray.length > 0) {
              const convertedResults: IdAdjustmentSearchResult[] = [];
              for (
                let i = 0;
                i < resultsArray.length && resultsCount < params.maxResults;
                i++
              ) {
                const wasmResult = resultsArray[i] as WasmIdAdjustmentSearchResult;
            ...
            if (now - lastProgressTime >= PROGRESS_INTERVAL_MS) {
              ...
              const progress: IdAdjustmentProgress = {
