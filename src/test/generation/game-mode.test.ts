...
function params(partial: Partial<GenerationParams>) {
  return {
    ...
    memoryLink: false,
    ...partial,
  } satisfies Pick<GenerationParams, 'version' | 'newGame' | 'noSave' | 'memoryLink'>;
}

describe('deriveDomainGameMode', () => {
  it('maps BW new game with save', () => {
    expect(deriveDomainGameMode(params({ version: 'B', newGame: true, noSave: false }))).toBe(DomainGameMode.BwNewGameWithSave);
  });

  it('maps BW new game without save', () => {
    expect(deriveDomainGameMode(params({ version: 'W', newGame: true, noSave: true }))).toBe(DomainGameMode.BwNewGameNoSave);
  });
  ...
  it('maps BW2 new game with save and no memory link', () => {
    expect(deriveDomainGameMode(params({ version: 'W2', newGame: true, noSave: false, memoryLink: false }))).toBe(DomainGameMode.Bw2NewGameNoMemoryLinkSave);
  });

  it('maps BW2 new game without save', () => {
    expect(deriveDomainGameMode(params({ version: 'B2', newGame: true, noSave: true }))).toBe(DomainGameMode.Bw2NewGameNoSave);
  });

  it('throws when memory link is set without save', () => {
    expect(() => deriveDomainGameMode(params({ version: 'W2', newGame: true, noSave: true, memoryLink: true })) ).toThrowError();
  });
});
